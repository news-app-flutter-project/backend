...
class ReaderController implements Controller {
  ...
  private findMostPopularWithCategory = asyncWrapper(async (req, res) => {
    ...
    try {
      const data = await readerService.findMostPopularForCategory(category);
  ...
  private findMostPopularWithCategoryAndAge = asyncWrapper(async (req, res) => {
    const response = customResponse(res);
    const { category, age } = req.body;
    console.log(category, age);
    try {
      const data = await readerService.findMostPopularForCategoryAndAge(
        category,
        age
      );
      return res.status(StatusCodes.OK).json({ result: true, data });
